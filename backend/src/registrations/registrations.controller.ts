...
export class RegistrationsController {
  ...
  async claimCandidate(
    ...
    const data = await this.registrationsService.claimCandidateForEvent(eventId, dto, user.id);
    return {
      success: true,
      data,
  ...
  @Get('events/:eventId/temp-credentials')
  @UseGuards(RolesGuard)
  @Roles(UserRole.SUPER_USER, UserRole.ADMINISTRATOR, UserRole.DCS)
  @ApiOperation({ summary: 'Admin: list temporary credentials generated by claims (expires after 30 days)' })
  async listTempCredentials(
    @Param('eventId') eventId: string,
    @Query('includeExpired', new DefaultValuePipe(false), ParseBoolPipe) includeExpired: boolean,
  ): Promise<ApiResponseDto<unknown>> {
    const data = await this.registrationsService.listTempCredentialLogs(eventId, includeExpired);
    return {
      success: true,
      data,
      message: 'Temporary credentials retrieved successfully',
    };
  }
